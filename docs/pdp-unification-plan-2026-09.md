# 상품 상세페이지(PDP) 통일 계획서 — sihoo.com.au (2026-09-08, **Alex 컨펌 완료 · 진행 중**)
...
> 2. 확정: 메타필드를 단일 소스로 읽는 **시리즈별 템플릿 3개(Doro · M · X) + 데스크 1개**와 **공통 섹션 라이브러리 13개**를 드래프트 테마에 만든다. 네 템플릿은 같은 섹션을 쓰고 순서·색 스킴·기본 토글만 다르다.
> 3. 4단계(디자인 토큰 → 섹션 라이브러리 → 상품별 콘텐츠 이관 → 검수·게시), 라이브 테마는 게시 순간까지 무접촉. §7 결정 8가지 확정(2026-09-08), P1 진행 중.
...
```
product.doro.json    ← Doro 7종 (C300 Pro V2, C300 Pro, S300, S100, C500, C100, M59AS)
product.m.json       ← M·V 시리즈 13종 (M57, M18, M57 Pro ×2, M18 Pro, M56, M16, M59, M57 footrest, Vito M90 ×2, M76, V1)
product.x.json       ← XALLKING 5종 (X5 Pro, X5S, X5C, X5F, X3 Pro) — 다크 스킴 기본
product.desk.json    ← Desker 2종 (스펙표 매핑만 다름)
```

네 템플릿은 **같은 섹션 라이브러리**를 쓴다. 다른 것은 섹션 순서 기본값, 색 스킴, 켜져 있는 섹션뿐이다(Alex 결정 Q1, 2026-09-08).
...
| **P3 콘텐츠 이관** | **전 상품 27개**, 한 상품씩 순차로: (1) 현재 페이지의 사진·이미지 카피·텍스트를 표로 분리 → (2) 텍스트는 메타필드/블록으로, 사진은 갤러리/스토리 이미지로 배치 → (3) 프리뷰 링크로 Alex 확인 → 다음 상품. 사진과 텍스트 설명이 섞이지 않도록 상품마다 전/후 표를 남긴다. 순서: Doro 7 → M 13 → X 5 → 데스크 2 | 상품별 이관 표(전/후) + 프리뷰 링크 | 5~8일, 본사 자산 회신에 따라 변동 |
...
## 7. 결정 (Alex 확정, 2026-09-08)

| # | 결정 |
|---|---|
| Q1 | **시리즈별 3개(Doro · M · X) + 데스크 1개** |
| Q2 | 1차 버튼 **브랜드 레드 #c20000**, 2차 검정 테두리 |
| Q3 | 버튼 **문장형**("Add to cart") — 전역 설정이라 사이트 전체 버튼이 함께 바뀜을 인지하고 승인 |
| Q4 | 세일 표기 **"Save $290 (37%)" 한 줄**, 모서리 배지 제거 |
| Q5 | HIGH DEMAND 박스는 **실제 지연 변형에만**, 표시될 때는 "1-5일 배송" 문구 숨김 |
| Q6 | 이미지 카피 → 텍스트 이관 **전 상품**, 한 상품씩 확인하며 순차 진행(사진·텍스트 혼동 방지) |
| Q7 | 자동 비교표 **포함** |
| Q8 | 커스텀 리뷰 섹션 **삭제**, 단 그 안의 리뷰 데이터는 **Loox 임포트용 CSV 로 먼저 추출**, 리뷰는 Loox 로 단일화 |
